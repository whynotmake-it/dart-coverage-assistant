...
function buildHeader(project: CoveredProject): string {
    const percentage = project.coverage ? getProjectPercentage(project) : undefined;
    const percentageCell = percentage ? `<td>${percentage.toFixed(2)}%</td>` : '<td>⚠️</td>';
    ...
    const badgeCell = percentage
        ? `<td>${buildBadgetableMd(project.name, Config.upperCoverageThreshold, Config.lowerCoverageThreshold, percentage)}</td>`
        : '<td>-</td>';
...
function getDiff(project: CoveredProject): number | undefined {
    ...
    const before = project.coverageBefore === null ? 0 : getLcovPercentage(project.coverageBefore);
    return current - before;
}

function buildBadgetableMd(name: string, upper: number, lower: number, percentage: number): string {
    const percentageString = percentage.toFixed(2) + "%";
    const alt = percentage >= upper ? "pass" : percentage >= lower ? "warning" : "fail";
    const color = percentage >= upper ? "success" : percentage >= lower ? "important" : "critical";
